...
def welcome():
    return (
        ...
        f"Last Years Hawaiian Precipatation Totals (Daily) <a href=\"/api/v1.0/precipitation\">/api/v1.0/precipitation<a><br/>"
        ...
        f"Minimum, Maximum, and Average Temperatures <a href=\"/api/v1.0/hawaii/yyyy-mm-dd/yyyy-mm-dd\">/api/v1.0/hawaii/yyy-mm-dd/yyyy-mm-dd<a><br>"
    )
...
def tobs():
    ...
    for date, observation in temps_station:
...
@app.route("/api/v1.0/hawaii/<prior_date>")
def hawaii1(prior_date, end_date='2017-08-23'):
    ...
    if Hawaii_dict['Min']: 
        return jsonify(Hawaii_stats)
    else:
        return jsonify({"error": f"Date {prior_date} is not formatted as YYYY-MM-DD or found."}), 404
  
@app.route("/api/v1.0/hawaii/<prior_date>/<end_date>")
